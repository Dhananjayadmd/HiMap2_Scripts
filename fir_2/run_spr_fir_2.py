...
def main(no_clusters, no_init,C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c, arch_desc, maxIter,skip_inter_or_intra, open_set_limit, entry_id,summary_log,initII,maxIterTime):
  ...
  DFG_GEN_KERNEL = DFG_GEN_HOME + '/applications/fir/'
  DFG_CLUSTRNG_KERNEL = DFG_CLUSTRNG_HOME + '/applications/fir/' + dir_name
  MAPPER_KERNEL = MAPPER_HOME + '/applications/clustered_arch/fir/'+ dir_name
...
##############################################################################################################################################
  ...
  print('\nGenerating DFG\n')
  #os.system('./run_pass.sh fir_filter')
  #os.system('dot -Tpdf fir_filter_INNERMOST_LN11_PartPredDFG.dot -o fir_filter_INNERMOST_LN11_PartPredDFG.pdf')
  os.system('cp fir_filter_unrolled_2_INNERMOST_LN11_PartPred_DFG_forclustering.xml '+DFG_CLUSTRNG_KERNEL)
  os.system('cp fir_filter_unrolled_2_INNERMOST_LN11_PartPred_DFG.xml '+ MAPPER_KERNEL)

  print('\nRunning DFG Clustering\n')
  os.chdir(DFG_CLUSTRNG_KERNEL)
  os.system('python ../../../dfg_clustering.py fir_filter_unrolled_2_INNERMOST_LN11_PartPred_DFG_forclustering.xml %s %s %s %s > log1.txt' % (no_clusters, ('precomputed'), no_init, cgra_cluster_r))
  os.system('dot -Tpdf inter_cluster.dot -o inter_cluster_graph_fir_%s_%s.pdf' % (no_clusters, no_init))
