...
##############################################################################################################################################
  ...
  os.system(HIMAP2_HOME+'/Morpher_CGRA_Mapper/build_legacy_spr/src/cgra_xml_mapper -m %s -d fir_filter_unrolled_2_INNERMOST_LN11_PartPred_DFG.xml -j %s -s %s -l %s -u %s -a %s -i %s -w %s -v %s > log.txt &' % (maxIter,HIMAP2_HOME+'/Morpher_CGRA_Mapper/json_arch/clustered_archs/'+arch_desc, skip_inter_or_intra, open_set_limit,HIMAP2_HOME+'/HiMap2_Scripts/'+summary_log, entry_id,initII, maxIterTime, HIMAP2_HOME+'/HiMap2_Scripts/Logs/legacy_fir.log'))
