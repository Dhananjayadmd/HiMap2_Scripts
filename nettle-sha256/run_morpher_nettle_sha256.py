...
def main(no_clusters, no_init,C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c, arch_desc, maxIter,skip_inter_or_intra, open_set_limit, entry_id,summary_log,initII,maxIterTime):
  ...
  DFG_CLUSTRNG_KERNEL = DFG_CLUSTRNG_HOME + '/applications/nettle-sha256/' + dir_name
  MAPPER_KERNEL = MAPPER_HOME + '/applications/clustered_arch/nettle-sha256/'+ dir_name
...
##############################################################################################################################################
  ...
  os.system(HIMAP2_HOME+'/Morpher_CGRA_Mapper/build_hierarchical/src/cgra_xml_mapper -m %s -d _nettle_sha256_compress_INNERMOST_LN1_PartPred_DFG.xml -j %s -s %s -l %s -u %s -a %s -i %s -w %s -v %s > log.txt &' % (maxIter,HIMAP2_HOME+'/Morpher_CGRA_Mapper/json_arch/clustered_archs/'+arch_desc, skip_inter_or_intra, open_set_limit,HIMAP2_HOME+'/HiMap2_Scripts/'+summary_log, entry_id, initII, maxIterTime, HIMAP2_HOME+'/HiMap2_Scripts/Logs/himap2_nettlesha.log'))
