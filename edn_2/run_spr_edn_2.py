...
##############################################################################################################################################
  ...
  os.system('cp jpegdct_unrolled_3_POST_LN111_PartPred_DFG_forclustering.xml '+DFG_CLUSTRNG_KERNEL)
  os.system('cp jpegdct_unrolled_3_POST_LN111_PartPred_DFG.xml '+ MAPPER_KERNEL)

  print('\nRunning DFG Clustering\n')
  os.chdir(DFG_CLUSTRNG_KERNEL)
  os.system('python ../../../dfg_clustering.py jpegdct_unrolled_3_POST_LN111_PartPred_DFG_forclustering.xml %s %s %s %s > log1.txt' % (no_clusters, ('precomputed'), no_init, cgra_cluster_r))
...
##############################################################################################################################################
  ...
  os.system(HIMAP2_HOME+'/Morpher_CGRA_Mapper/build_hierarchical_spr/src/cgra_xml_mapper -m %s -d jpegdct_unrolled_3_POST_LN111_PartPred_DFG.xml -j %s -s %s -l %s -u %s -a %s -i %s -w %s -v %s > log.txt &' % (maxIter,HIMAP2_HOME+'/Morpher_CGRA_Mapper/json_arch/clustered_archs/'+arch_desc, skip_inter_or_intra, open_set_limit,HIMAP2_HOME+'/HiMap2_Scripts/'+summary_log, entry_id, initII, maxIterTime, HIMAP2_HOME+'/HiMap2_Scripts/Logs/himap2_edn.log'))
