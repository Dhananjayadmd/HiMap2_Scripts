...
def main(no_clusters, no_init,C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c, arch_desc, maxIter,skip_inter_or_intra, open_set_limit, entry_id,summary_log,initII,maxIterTime):
  ...
  DFG_GEN_KERNEL = DFG_GEN_HOME + '/applications/matrix_multiply/'
  DFG_CLUSTRNG_KERNEL = DFG_CLUSTRNG_HOME + '/applications/matrix_multiply/' + dir_name
  MAPPER_KERNEL = MAPPER_HOME + '/applications/clustered_arch/matrix_multiply/'+ dir_name
...
##############################################################################################################################################
  ...
  print('\nGenerating DFG\n')
  #os.system('./run_pass.sh matrix_multiply')
  os.system('dot -Tpdf matrix_multiply_INNERMOST_LN1_PartPredDFG.dot -o matrix_multiply_INNERMOST_LN1_PartPredDFG.pdf')
  os.system('cp matrix_multiply_unrolled_2_INNERMOST_LN1_PartPred_DFG_forclustering.xml '+DFG_CLUSTRNG_KERNEL)
  os.system('cp matrix_multiply_unrolled_2_INNERMOST_LN1_PartPred_DFG.xml '+ MAPPER_KERNEL)

  print('\nRunning DFG Clustering\n')
  os.chdir(DFG_CLUSTRNG_KERNEL)
  os.system('python ../../../dfg_clustering.py matrix_multiply_unrolled_2_INNERMOST_LN1_PartPred_DFG_forclustering.xml %s %s %s %s > log1.txt' % (no_clusters, ('precomputed'), no_init, cgra_cluster_r))
  os.system('dot -Tpdf inter_cluster.dot -o inter_cluster_graph_matrix_multiply_%s_%s.pdf' % (no_clusters, no_init))
...
##############################################################################################################################################
  ...
  os.system(HIMAP2_HOME+'/Morpher_CGRA_Mapper/build_hierarchical_spr/src/cgra_xml_mapper -m %s -d matrix_multiply_unrolled_2_INNERMOST_LN1_PartPred_DFG.xml -j %s -s %s -l %s -u %s -a %s -i %s -w %s -v %s > log.txt &' % (maxIter,HIMAP2_HOME+'/Morpher_CGRA_Mapper/json_arch/clustered_archs/'+arch_desc, skip_inter_or_intra, open_set_limit,HIMAP2_HOME+'/HiMap2_Scripts/'+summary_log, entry_id, initII, maxIterTime, HIMAP2_HOME+'/HiMap2_Scripts/Logs/himap2_matrix_multiply.log'))
