...
def main(no_clusters, no_init,C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c, arch_desc, maxIter,skip_inter_or_intra, open_set_limit, entry_id,summary_log,initII,maxIterTime):
  ...
  DFG_GEN_KERNEL = DFG_GEN_HOME + '/applications/invertmat/'
  DFG_CLUSTRNG_KERNEL = DFG_CLUSTRNG_HOME + '/applications/invertmat/' + dir_name
  MAPPER_KERNEL = MAPPER_HOME + '/applications/clustered_arch/invertmat/'+ dir_name
  EXECTIME_SUMMARY = HIMAP2_HOME + '/HiMap2_Scripts/exec_time/invertmat/' 
...
##############################################################################################################################################
  ...
  os.system('cp invert_matrix_general_more_unrolled_INNERMOST_LN1_PartPred_DFG_forclustering.xml '+DFG_CLUSTRNG_KERNEL)
  os.system('cp invert_matrix_general_more_unrolled_INNERMOST_LN1_PartPred_DFG.xml '+ MAPPER_KERNEL)

  print('\nRunning DFG Clustering\n')
  os.chdir(DFG_CLUSTRNG_KERNEL)
  os.system('python ../../../dfg_clustering.py invert_matrix_general_more_unrolled_INNERMOST_LN1_PartPred_DFG_forclustering.xml %s %s %s %s > log1.txt' % (no_clusters, ('precomputed'), no_init, cgra_cluster_r))
  #os.system('dot -Tpdf inter_cluster.dot -o inter_cluster_graph_invertmat_%s_%s.pdf' % (no_clusters, no_init))
